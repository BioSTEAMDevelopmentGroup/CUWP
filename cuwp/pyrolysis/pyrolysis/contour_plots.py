...
__all__ = ('carbonyield_at_carbonprice_processing_capacity',
           '_contour_subplots', 
            '_plot_contour_single_metric',
             'carbonyield_price', 
             'plot_carbonyield_price',
             'carbonyield_dieselprice',
            'plot_carbonyield_dieselprice',
            'carbonyield_temperature_FEDI',
            'plot_carbonyield_temperature_FEDI')
...
def carbonyield_at_carbonprice_processing_capacity(carbon_yield, carbon_price, processing_capacity):
    IRR_array = np.zeros_like(processing_capacity)  # Variable to get
    for i, value in enumerate(processing_capacity):
        context_sys, tea, NPV , IRR_value , Fedi_val, GWP_revenue  = create_system(
            processing_capacity=value, 
            carbon_conversion=carbon_yield, 
            activated_carbon_price=carbon_price,
        ) 
        context_sys.simulate()
        IRR_array[i] = IRR_value
    return IRR_array

def carbonyield_at_dieselprice_processing_capacity(carbon_yield, diesel_price , processing_capacity):
    IRR_array = np.zeros_like(processing_capacity)  # Variable to get
    for i, value in enumerate(processing_capacity):
        context_sys, tea, NPV , IRR_value, Fedi_val , GWP_revenue = create_system(
            processing_capacity=value, 
...
def FEDI_carbonyield_temperature_FEDI(carbon_yield, temperature_val , titer):
    ...
    for i, value in enumerate(titer):
        context_sys, tea, NPV , IRR_value, Fedi_val, GWP_revenue  = create_system(
            processing_capacity=value,
            carbon_conversion=carbon_yield, 
            reactor_temperature=temperature_val , 
        ) 
        context_sys.simulate()
        FEDI_array[i] = Fedi_val
    return FEDI_array


def carbonyield_price(file = 'default.npz', n = 10):

    processing_capacity = np.array([2500,4166,6250])  # puntos 
    ...
    X, Y, Z = bst.plots.generate_contour_data(
        carbonyield_at_carbonprice_processing_capacity,
        xlim=xlim, ylim=ylim,
        args=(processing_capacity,),
...
def plot_carbonyield_price(file = 'default.npz'):
    output_folder = 'results'
...
def carbonyield_dieselprice(file = 'default.npz', n = 10):
    processing_capacity = np.array([2500,4166,6250])  # puntos 
    carbon_conversion = 55
    ...
    X, Y, Z = bst.plots.generate_contour_data(
        carbonyield_at_dieselprice_processing_capacity,
        xlim=xlim, ylim=ylim,
        args=(processing_capacity,),
        n = n,)  # numero de puntos a evaluar por en x  y y 
    # Guarda vectores y despues cargarlos 
...
def plot_carbonyield_dieselprice(file = 'default.npz'):
    output_folder = 'results'
    os.makedirs(output_folder, exist_ok=True)
...
def carbonyield_temperature_FEDI(file = 'default.npz', n = 10):
...
def plot_carbonyield_temperature_FEDI(file = 'default.npz'):
    output_folder = 'results'
    os.makedirs(output_folder, exist_ok=True)
