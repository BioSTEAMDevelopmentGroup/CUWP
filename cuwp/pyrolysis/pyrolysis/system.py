...
class Reactor(bst.Unit):
    ...
    def _init(self, Treac = None, Preac = None, velocity = None, carbon_conversion = None,
               processing_capacity = None): # diameter, length):
        # The _init methods adds input parameters for unit creation
        self.Treac = Treac #:  Operation temperature [K]
    ...
    def _get_MW(self):
        self.MW_list= self.chemicals.MW
    ...
    def _run(self):
        ...
        self.initial_conditions.extend([0] * (len(self.chemicals)-8))  # 106
...
class Naptha_PartialOxidation(bst.Unit):
    ...
    def _run(self):
        ...
        self.atoms_matrix = self.chemicals.formula_array
...
class Hydrotreatment(bst.Unit):
    ...
    def __init__(self, ID='', ins=None, outs=(), T= None, P=None, 
                    processing_capacity = None, epsilon = 1e-8):
        super().__init__(ID, ins, outs)
        self.Treac = T  # Temperatura en K
        self.Preac = P  # Presión en pascales
        self.epsilon = epsilon # Minimum amount of flowrate for each chemical
...
def create_system(
        processing_capacity=6250, # kg/hr
        carbon_conversion=55, # %
        activated_carbon_price=3.5,
        diesel_price=1137.50/1000,
        reactor_temperature=550 + 273.15,
    ):
    with bst.System() as sys:
        ...
        Carbon =  bst.Stream('CarbonActivated', price = activated_carbon_price)  # https://businessanalytiq.com/procurementanalytics/index/activated-carbon-prices/
        R2 = Reactor('MainReactor', outs= ('stream1', Metal, Carbon), 
                    Treac = reactor_temperature, Preac=101325, velocity= 0.0001, processing_capacity = processing_capacity,
                    carbon_conversion = carbon_conversion, ins=(R1-0)) 
        ...
        Diesel = bst.Stream('Diesel', price= diesel_price)
    ...
    sys.set_tolerance(mol=1e-2, rmol=1e-2)
    sys.simulate()
    osbl_units = bst.get_OSBL(sys.cost_units)
    tea = PyrolyisisTEA(system=sys, 
    ...
    sys.define_process_impact(
        ...
        basis = 'kg',
        inventory =  lambda: bst.F.emissions.imass['CO2'] * sys.operating_hours,
        CF =  1.0 
    )
    sys.define_process_impact(
        key = GWP ,
        ...
        basis = 'kg',
        inventory =  lambda: bst.F.OxRedox.outs[0].imass['CO2'] * sys.operating_hours,
        CF =  1.0 
    )
    
    # Displacement allocation [kg-CO2e / kg-ethanol]
    GWP_displacement = sys.get_net_impact(key=GWP) / sys.get_mass_flow(bst.F.CarbonActivated)
    # Energy allocation by gasoline gallon equivalent (GGE)
    GWP_per_GGE = sys.get_property_allocated_impact(
    ...
    combustion_rxns = bst.settings.chemicals.get_combustion_reactions()
    final_flowsa = bst.F.MainReactor.final_flows[-1,:]
    # Salida de reactor
    flows_total =  [] 
    for i ,c in enumerate(bst.settings.chemicals): 
        if i > 2 and i < len(bst.settings.chemicals)-2:
    ...
    return sys, tea, NPV , IRR_val, Fedi_val, GWP_revenue
